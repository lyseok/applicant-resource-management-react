...
export default function AdminDashboard() {
  ...
  const memberStats = activeMembers.map((member) => {
    ...
    const inProgressTasks = memberTasks.filter(
      (task) => task.taskStatus === 'PEND-002'
    );
    const todoTasks = memberTasks.filter(
      (task) => task.taskStatus === 'PEND-001'
    );
  ...
  const filteredTasks = tasks.filter((task) => {
    const matchesAssignee =
      selectedAssignee === 'all' || task.userId === selectedAssignee;
    ...
    const matchesPriority =
      priorityFilter === 'all' || task.priorityCode === priorityFilter;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="bg-white shadow-sm border border-gray-200">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-gray-900 flex items-center">
                <TrendingUp className="w-5 h-5 mr-2 text-purple-600" />팀 성과
                분석
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    return (
                      ...
                      >
                        ...
                        {task.progressRate && (
                          <div className="mt-2">
                            <Progress
                              value={Number.parseInt(task.progressRate)}
                              className="h-1"
                            />
                          </div>
                        )}
