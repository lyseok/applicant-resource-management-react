...
import { X, Save, Trash2, MessageCircle } from "lucide-react"
import {
...
export default function ProjectSettingsModal({ isOpen, onClose, project }) {
  ...
  const [projectStatus, setProjectStatus] = useState(project?.projectStatus || "계획중")
  const [projectContents, setProjectContents] = useState(project?.projectContents || "")
  const [visibleTabs, setVisibleTabs] = useState(project?.visibleTabs || ALL_TABS.map((tab) => tab.id))
  const [chatroomName, setChatroomName] = useState(project?.chatroomName || "") // New state for chatroom name
  ...
  const handleDeleteProject = async () => {
    if (!project?.prjNo) return
    if (
      window.confirm(`정말로 프로젝트 '${project.projectName}'을(를) 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`)
    ) {
      try {
  ...
  const handleTabToggle = (tabId, checked) => {
    setVisibleTabs((prev) => (checked ? [...prev, tabId] : prev.filter((id) => id !== tabId)))
  }
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-6 space-y-6 overflow-y-auto flex-1">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">프로젝트 상태</label>
              ...
              <div>
                ...
                <Textarea value={projectContents} onChange={(e) => setProjectContents(e.target.value)} rows={4} />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">채팅방 이름</label>
            ...
            <div className="space-y-4 border-t border-gray-200 pt-6 mt-6">
              <h3 className="text-sm font-medium text-red-700">위험 구역</h3>
